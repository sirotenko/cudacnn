...
for k=1:(cnet.numLayers-cnet.numFLayers) 
    if(rem(k,2)) %Parity check
        ...
        if(~isempty(cnet.SLayer{k}.WS))
            warning('Weights and biases in pooling layers are omitted in CNN ver 2.0. Weights in SLayer ignored.');
        end
        if(~isempty(cnet.SLayer{k}.BS))
            warning('Weights and biases in pooling layers are omitted in CNN ver 2.0. Biases in SLayer ignored.');
        end
        ...
        cnet_20.layers{k}.TransferFunc = uint32(get_trasfer_func_enum(cnet.SLayer{k}.TransfFunc));
        cnet_20.layers{k}.SXRate = uint32(cnet.SLayer{k}.SRate);
        cnet_20.layers{k}.SYRate = uint32(cnet.SLayer{k}.SRate);
        cnet_20.layers{k}.LayerType = uint32(1);
    else
    %C-layer      
...
%         cnet_20.layers{k}.B = cellfun(@(x) transp(precisionFn(x)),cnet.CLayer{k}.BC,'UniformOutput',0);
        ...
        cnet_20.layers{k}.LayerType = uint32(0);
    end
end
...
for k=(cnet.numLayers-cnet.numFLayers+1):cnet.numLayers
    ...
    cnet_20.layers{k}.LayerType = uint32(2);
    cnet_20.layers{k}.TransferFunc = uint32(get_trasfer_func_enum(cnet.FLayer{k}.TransfFunc));
end
end
