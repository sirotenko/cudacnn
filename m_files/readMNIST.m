function [I,labels,I_test,labels_test] = readMNIST(num, varargin)
...
if ~isempty(varargin)
    path_base = [varargin{1}];
else
    path_base = '.\MNIST\';
end
path = [path_base 'train-images.idx3-ubyte'];
...
%============Loading labels
path = [path_base 'train-labels.idx1-ubyte'];
fid = fopen(path,'r','b');  %big-endian
magicNum = fread(fid,1,'int32');    %Magic number
if(magicNum~=2049) 
    display('Error: cant find magic number');
...
path = [path_base 't10k-images.idx3-ubyte'];
fid = fopen(path,'r','b');  
...
%============Test labels
path = [path_base 't10k-labels.idx1-ubyte'];
fid = fopen(path,'r','b');  
magicNum = fread(fid,1,'int32');    
if(magicNum~=2049) 
    display('Error: cant find magic number');
